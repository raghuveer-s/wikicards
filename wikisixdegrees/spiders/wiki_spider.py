...
class WikiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        pageItem = WikiPageItem(url=url, title=title, content=content, date_visited=date_visited, followed=True, outgoing_links=links)
        yield pageItem
        ...
        if response.request.meta['depth'] < self.custom_settings['DEPTH_LIMIT']:
            ...
            for link in links:
                yield WikiPageItem(url=link.url, title=None, content=None, date_visited=None, followed=False)
